...
class PurchaseService:
    ...
    @transaction.atomic
    def create(self, project_id: int, user_id: int) -> Purchase:
        purchase_obj = Purchase.objects.create(project_id=project_id, created_by_id=user_id)
        ...
        self.log_service.create(obj_type='Purchase',
                                obj_id=purchase_obj.id,
                                action='Create',
                                created_by_id=user_id)

    @transaction.atomic
    def update(self, purchase_id: int, validated_data: dict, user_id: int) -> Purchase:
        ...
        purchase_obj.updated_by_id = user_id
        purchase_obj.save()
    ...
    def export_xlsx(self, purchase_id: int) -> bytes:
        ...
        with NamedTemporaryFile() as tmp:
            ...
            for no, purchase in enumerate(purchase_obj.purchases.all()):
                ...
                ws.cell(row=row, column=15, value=purchase.offer.request.amount *
                        purchase.offer.count*purchase.offer.price)
                ...
                ws.cell(row=row, column=19, value=purchase.offer.request.amount *
                        purchase.offer.count*(purchase.price_buy-purchase.offer.price))
